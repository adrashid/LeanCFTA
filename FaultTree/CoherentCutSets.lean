...
/-
===========================================================
7. Coverage of Semantic Cut Sets
===========================================================
-/
...
-/
theorem cutSets_cover [DecidableEq Event]
    (t : CFTree Event)
    (C : CutSet Event)
    (h : IsCutSet C t) :
  ...
  | gate g ts =>
      cases g with
      | andGate =>
          have hAnd : IsCutSet C (AND ts) := by
            simpa [AND] using h

          obtain ⟨D, hDGenerated, hDSubset⟩ :=
            andCutSetsList_cover ts C hAnd

          refine ⟨D, ?_, hDSubset⟩
          simpa [cutSets, andCutSetsList] using hDGenerated

      | orGate =>
          have hOr : IsCutSet C (OR ts) := by
            simpa [OR] using h

          obtain ⟨D, hDGenerated, hDSubset⟩ :=
            orCutSetsList_cover ts C hOr

          refine ⟨D, ?_, hDSubset⟩
          simpa [cutSets, orCutSetsList] using hDGenerated

/--
If an AND-list is satisfied, its active-event set contains
a cut set generated for that list.
-/
theorem andCutSetsList_cover [DecidableEq Event]
    (ts : List (CFTree Event))
    (C : CutSet Event)
    (h : IsCutSet C (AND ts)) :
    ∃ D, D ∈ andCutSetsList ts ∧ D ⊆ C := by
  cases ts with
  | nil =>
      refine ⟨∅, ?_, ?_⟩
      · simp [andCutSetsList, andCombine]
  ...
  | cons t ts =>
      have hParts :
          IsCutSet C t ∧ IsCutSet C (AND ts) := by
        unfold IsCutSet at h ⊢
        simpa [eval_AND, evalAll] using h

      obtain ⟨D₁, hD₁Generated, hD₁Subset⟩ :=
        cutSets_cover t C hParts.1

      obtain ⟨D₂, hD₂Generated, hD₂Subset⟩ :=
        andCutSetsList_cover ts C hParts.2

      refine ⟨D₁ ∪ D₂, ?_, ?_⟩
      · rw [mem_andCutSetsList_cons]
        exact
          ⟨D₁, hD₁Generated,
           D₂, hD₂Generated,
           rfl⟩
      · exact Finset.union_subset hD₁Subset hD₂Subset

/--
If an OR-list is satisfied, its active-event set contains
a cut set generated by one of its children.
-/
theorem orCutSetsList_cover [DecidableEq Event]
    (ts : List (CFTree Event))
    (C : CutSet Event)
    (h : IsCutSet C (OR ts)) :
    ∃ D, D ∈ orCutSetsList ts ∧ D ⊆ C := by
  cases ts with
  | nil =>
      unfold IsCutSet at h
      simp [eval_OR, evalAny] at h

  | cons t ts =>
      have hCases :
          IsCutSet C t ∨ IsCutSet C (OR ts) := by
        unfold IsCutSet at h ⊢
        simpa [eval_OR, evalAny] using h

      cases hCases with
      | inl hHead =>
          obtain ⟨D, hDGenerated, hDSubset⟩ :=
            cutSets_cover t C hHead

          refine ⟨D, ?_, hDSubset⟩
          rw [mem_orCutSetsList]
          exact ⟨t, by simp, hDGenerated⟩

      | inr hTail =>
          obtain ⟨D, hDGenerated, hDSubset⟩ :=
            orCutSetsList_cover ts C hTail

          refine ⟨D, ?_, hDSubset⟩
          rw [mem_orCutSetsList] at hDGenerated ⊢

          obtain ⟨s, hsMember, hDs⟩ := hDGenerated
          exact ⟨s, by simp [hsMember], hDs⟩
...
/--
Every cut set returned by `minimalCutSets` is globally minimal
according to the semantic definition `MinimalCutSet`.
-/
theorem minimalCutSets_globally_minimal [DecidableEq Event]
    (t : CFTree Event)
    (C : CutSet Event)
    (h : C ∈ minimalCutSets t) :
    MinimalCutSet C t := by
  constructor
  · exact minimalCutSets_sound t C h

  · intro D hDC hDcut

    obtain ⟨E, hEGenerated, hED⟩ :=
      cutSets_cover t D hDcut

    have hEC : E ⊂ C := by
      constructor
      · exact hED.trans hDC.1
      · intro hCE
        have hCD : C ⊆ D :=
          hCE.trans hED
        exact hDC.2 hCD
    ...
    exact
      minimalCutSets_generated_minimal
        t C h E hEGenerated hEC


/--
Every semantically minimal cut set is returned by
`minimalCutSets`.
-/
theorem minimalCutSets_complete_semantic [DecidableEq Event]
    (t : CFTree Event)
    (C : CutSet Event)
    (h : MinimalCutSet C t) :
    C ∈ minimalCutSets t := by
  obtain ⟨D, hDGenerated, hDC⟩ :=
    cutSets_cover t C h.1

  have hDCEqual : D = C := by
    apply Finset.Subset.antisymm
    · exact hDC
    · by_contra hNotSubset

      have hProper : D ⊂ C := by
        exact ⟨hDC, hNotSubset⟩

      have hDCut : IsCutSet D t :=
        cutSets_sound t D hDGenerated

      exact h.2 D hProper hDCut

  subst D

  unfold minimalCutSets
  rw [mem_minimizeCutSets_iff]
  constructor
  · exact hDGenerated
  · intro E hEGenerated hEC

    have hECut : IsCutSet E t :=
      cutSets_sound t E hEGenerated

    exact h.2 E hEC hECut


/--
A finite event set is returned by `minimalCutSets` exactly when
it is a semantic minimal cut set.
-/
theorem mem_minimalCutSets_iff_semantic [DecidableEq Event]
    (t : CFTree Event)
    (C : CutSet Event) :
    C ∈ minimalCutSets t ↔ MinimalCutSet C t := by
  constructor
  · intro h
    exact minimalCutSets_globally_minimal t C h
  · intro h
    exact minimalCutSets_complete_semantic t C h
